...
const SlideShow = () => {
  // 假设有 5 张图片
  const images = Array.from({ length: 15 }, (_, i) => `/MagazinePage/Slide/${String(i + 1).padStart(2, '0')}.png`);
  ...
  return (
    <div className="relative w-full max-w-4xl overflow-hidden">
      {/* 图片列表 */}
      <div
        className="flex transition-transform duration-500 px-12 space-x-12"
      ...
      >
        {images.map((src, index) => (
          <img
            ...
            src={src}
            alt={`Slide ${index + 1}`}
            className="w-2/5 mx-2 rounded-lg" // 设置每张图片的宽度
            style={{ flexShrink: 0 }}
          />
        ))}
      </div>
